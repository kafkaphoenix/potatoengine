...
motores que vimos en la \figurename~\ref{engine_distribution}. 

\section*{2.1. ¿Qué es un motor de juegos?}\addcontentsline{toc}{section}{2.1. ¿Qué es un motor de juegos?}\label{sec:game-engine}

Un motor de juegos se compone de diferentes piezas, pero podemos ver las principales en la \figurename~\ref{engine_components}.

\begin{figure}[h!]
    \centering
    \smartdiagram[constellation diagram]{Motor, Motor Matemático, Motor Gráfico, Motor Físicas, Motor Entidades}
    \caption{Partes de un motor}
    \label{engine_components}
\end{figure}

Lo primero que necesitamos para poder crear un juego es crear una ventana donde podamos \textit{visualizar la escena}, 
de esto se encargará el \textbf{motor gráfico}. Esta escena tendrá elementos que llamaremos \textit{entidades} como puede ser el jugador,
...
\begin{itemize}
    ...
    \item Plantillas para tomar una idea del uso del motor
    \item Herramientas de análisis para mejorar el rendimiento, ver logs e inspeccionar elementos
    \item La opción de exportar a diferentes plataformas
    \item La opción de crear juegos tanto en \gls{2d-es} como en \gls{3d-es}
\end{itemize}

En consecuencia, este proyecto se ha desarrollado usando el lenguaje \textit{C++}, estándar en el desarrollo de videojuegos por su eficiencia, junto a cmake\cite{cmake} que
permite la compilación multiplataforma del proyecto. Además las siguientes librerías se han usado para cubrir las necesidades del motor:
\begin{itemize}
