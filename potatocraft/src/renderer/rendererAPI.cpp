...
namespace potatoengine {
	...
	void APIENTRY message_callback(GLenum  source, GLenum  type, uint32_t id, GLenum  severity, GLsizei length, GLchar const* message, void const* user_param)
	{
		...
		switch (severity) {
			case GL_DEBUG_SEVERITY_HIGH:         CORE_CRITICAL(info); return;
			case GL_DEBUG_SEVERITY_MEDIUM:       CORE_ERROR(info); return;
			case GL_DEBUG_SEVERITY_LOW:          CORE_WARN(info); return;
			case GL_DEBUG_SEVERITY_NOTIFICATION: CORE_TRACE(info); return;
		}
