...
struct CNoise {
    ...
    CNoise() = default;
    CNoise(std::string type, int seed, int octaves, float frequency, float persistence, float lacunarity, int amplitude, bool positive)
    ...
    void setNoiseType() {
        if (_type == "simplex") {
            ...
            throw std::runtime_error("Unknown collider type " + _type);
        }
    }
