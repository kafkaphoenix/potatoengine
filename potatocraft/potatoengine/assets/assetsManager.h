...
class AssetsManager {
   public:
    ...
    const std::shared_ptr<Type>& reload(std::string_view id, Args&&... args) {
        ...
        ENGINE_ASSERT(std::holds_alternative<std::shared_ptr<Type>>(maybeAsset->second), "Asset {} has an incompatible type!", id);
    ...
    const std::unordered_map<std::string, AssetTypes>& getAssets() const noexcept {
        return m_assets;
    }
