...
#include "demos/serializers/settings.h"
#include "demos/ui/imdebugger.h"

namespace demos {

GameState::GameState(std::weak_ptr<engine::AssetsManager> am, std::weak_ptr<engine::Renderer> r, Settings&& s) : State("GameState"), m_sceneManager(am, r), m_assetsManager(am), m_renderer(r), m_settings(std::move(s)) {
...
void GameState::onAttach() {
    if (not m_settings.activeScene.empty()) {
        ...
        manager->load<engine::Scene>(m_settings.activeScene, m_settings.activeScenePath);
...
void GameState::onUpdate(engine::Time ts) {
    if (m_settings.reloadScene and m_sceneManager.getActiveScene() != m_settings.activeScene) {
        const auto& manager = m_assetsManager.lock();
        APP_ASSERT(manager, "AssetsManager is null!");
...
std::unique_ptr<engine::State> GameState::Create(std::weak_ptr<engine::AssetsManager> am, std::weak_ptr<engine::Renderer> r, Settings&& s) {
