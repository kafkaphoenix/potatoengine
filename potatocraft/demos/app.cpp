...
class Demos : public engine::Application {
   public:
    Demos(engine::Config&& c, engine::CLArgs&& args) : engine::Application(std::move(c), std::move(args)) {
        ...
        pushState(GameState::Create(std::weak_ptr<engine::AssetsManager>(m_assetsManager), std::weak_ptr<engine::Renderer>(m_renderer), serializers::load_settings()));
        APP_INFO("State loaded!");
    }

    ~Demos() override {
        APP_WARN("Deleting Demos application");
    }
};
