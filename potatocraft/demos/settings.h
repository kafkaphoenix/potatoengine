...
struct Settings {
    ...
    std::vector<const char*> scenes{"Sponza", "Dabrovic Sponza", "Cubes", "Lowpoly City", "Plane Terrain", "Primitives", "Empty"};
    std::vector<const char*> scenesPaths{"assets/scenes/sponza_scene.json", "assets/scenes/dabrovic_sponza_scene.json", "assets/scenes/cubes_scene.json", "assets/scenes/lowpoly_city_scene.json", "assets/scenes/plane_terrain_scene.json", "assets/scenes/primitives_scene.json", "assets/scenes/empty_scene.json"};
    std::vector<const char*> cursorModes{"Normal", "Hidden", "Disabled"};
    std::vector<const char*> debugLevels{"Trace", "Info", "Warn", "Error", "Critical"};
};
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(Settings, appName, root, logFilePath, windowIconPath, windowWidth, windowHeight, depthBits, refreshRate, fullscreen, primaryMonitor, vSync, resizable, openglMajorVersion, openglMinorVersion, cursorIconPath, cursorMode, debugEnabled, debugLevel, displayFPS, clearColor, clearDepth, activeScene, activeScenePath);
}