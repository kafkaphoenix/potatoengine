...
void movementSystem(entt::registry& r, Time ts) {
  r.view<CTransform, CRigidBody, CUUID>().each(
    [&](entt::entity e, CTransform& cTransform, const CRigidBody& cRigidBody,
      ...
      CInput* cInput = r.try_get<CInput>(e);

      if (cCamera and cCamera->mode == CCamera::Mode::_3D) {
      ...
      } else if (cInput and cRigidBody.isKinematic) {
        if (not Application::Get().isDebugging()) {
          float speed = ts * cInput->translationSpeed;
        ...
        // TODO REMOVE THIS
        // TODO this should throw an error think how to make thinks move maybe
        // animation
        if (cRigidBody.isKinematic) { // TODO just temporal for cubes testing
          cTransform.rotation =
            glm::angleAxis(glm::radians(1.f), glm::vec3(0.f, 1.f, 0.f)) *
            cTransform.rotation;
        }
      }
    });
