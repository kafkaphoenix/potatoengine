...
std::vector<std::vector<glm::vec3>> generateBiomesTextures(std::vector<std::vector<float>> heights, int amplitude,
...
CMesh generateTriangleMesh(
    ...
    const std::optional<std::reference_wrapper<std::vector<std::vector<glm::vec3>>>> biomes = std::nullopt) {
  CMesh mesh;
  int totalVertices =
      (chunkSize + 1) *
      (chunkSize + 1); // Triangles share vertices making impossible to use more than one texture for all the mesh
  ...
  for (int row = 0; row < chunkSize + 1; ++row) {
    for (int col = 0; col < chunkSize + 1; ++col) {
      ...
      glm::vec3 normal = std::move(calculateQuadNormal(
          col, row, heights)); // TODO maybe this normal is not correct and should call the other method
...
void addQuadVertexData(std::vector<TerrainVertex>& vertices, std::vector<uint32_t>& indices,
                       const std::array<glm::vec3, 4>& positions, glm::vec3 normal,
                       const std::array<glm::vec2, 4>& textureCoordinates, const std::array<glm::vec3, 4>& colors) {
...
void addTriangleVertexData(std::vector<TerrainVertex>& vertices, std::vector<uint32_t>& indices, glm::vec3 first,
                           glm::vec3 second, glm::vec3 third, glm::vec3 normal, glm::vec2 firstTextureCoordinates,
                           glm::vec2 secondTextureCoordinates, glm::vec2 thirdTextureCoordinates, glm::vec3 firstColor,
...
std::array<glm::vec3, 4> calculateQuadPositions(int col, int row, const std::vector<std::vector<float>>& heights,
                                                int blockSize) {
  std::array<glm::vec3, 4> positions;
  positions[0] = {static_cast<float>(col) * blockSize, heights[row][col] * blockSize,
                  static_cast<float>(row) * blockSize}; // top left
  positions[1] = {static_cast<float>(col) * blockSize, heights[row + 1][col] * blockSize,
                  static_cast<float>(row + 1) * blockSize}; // bottom left
  positions[2] = {static_cast<float>(col + 1) * blockSize, heights[row][col + 1] * blockSize,
                  static_cast<float>(row) * blockSize}; // top right
  positions[3] = {static_cast<float>(col + 1) * blockSize, heights[row + 1][col + 1] * blockSize,
...
void addQuad(std::vector<TerrainVertex>& vertices, std::vector<uint32_t>& indices, int col, int row, int chunkSize,
  ...
  if (duplicateVertices) {
    addTriangleVertexData(vertices, indices, positions[0], positions[1], positions[2],
                          calculateTriangleNormal(positions[0], positions[1], positions[2]), textureCoordinates[0],
                          textureCoordinates[1], textureCoordinates[2], colors[0], colors[1], colors[2]);
    addTriangleVertexData(vertices, indices, positions[2], positions[1], positions[3],
                          calculateTriangleNormal(positions[2], positions[1], positions[3]), textureCoordinates[2],
                          textureCoordinates[1], textureCoordinates[3], colors[2], colors[1], colors[3]);
  } else {
...
CMesh generateQuadMesh(
    int chunkSize, int blockSize, bool duplicateVertices, CTexture::DrawMode drawMode,
    const std::vector<std::vector<float>>& heights,
    const std::optional<std::reference_wrapper<std::vector<std::vector<glm::vec3>>>> biomes = std::nullopt) {
  CMesh mesh;
...
CMesh generateTerrain(
    CChunkManager::MeshType meshType, CChunkManager::MeshAlgorithm meshAlgorithm, int chunkSize, int blockSize,
  ...
  if (meshType == CChunkManager::MeshType::Plane) {
    if (meshAlgorithm == CChunkManager::MeshAlgorithm::SimplifiedTriangle) {
      if (biomes.has_value()) {
        ...
        ENGINE_ASSERT(false, "Draw mode {} not supported for mesh type {} and mesh algorithm {}",
                      static_cast<int>(drawMode), static_cast<int>(meshType), static_cast<int>(meshAlgorithm));
      }
...
void terrainSystem(entt::registry& reg) { // TODO transform
  ...
  reg.view<CChunkManager, CTexture, CNoise, CUUID>().each([&](entt::entity e, CChunkManager& cChunkManager,
                                                              const CTexture& cTexture, const CNoise& cNoise,
                                                              const CUUID& cUUID) {
      ...
      for (int row = -cChunkManager.width; row <= cChunkManager.width; ++row) {
        for (int col = -cChunkManager.height; col <= cChunkManager.height; ++col) {
          ...
          if (cChunkManager.useBiomes) {
            ...
            } else if (cTexture.drawMode == CTexture::DrawMode::TEXTURE_ATLAS or
                       cTexture.drawMode == CTexture::DrawMode::TEXTURE_ATLAS_BLEND or
                       cTexture.drawMode == CTexture::DrawMode::TEXTURE_ATLAS_BLEND_COLOR) {
              biomes = generateBiomesTextures(heights, cNoise.amplitude, *cTextureAtlas);
            }
            chunk.terrainMesh =
                std::move(generateTerrain(cChunkManager.meshType, cChunkManager.meshAlgorithm, cChunkManager.chunkSize,
                                          cChunkManager.blockSize, cTexture.drawMode, heights, biomes));
          } else {
            chunk.terrainMesh =
                std::move(generateTerrain(cChunkManager.meshType, cChunkManager.meshAlgorithm, cChunkManager.chunkSize,
                                          cChunkManager.blockSize, cTexture.drawMode, heights));
          }
          chunk.transform.position =
              glm::vec3{static_cast<float>(col) * cChunkManager.chunkSize * cChunkManager.blockSize, 0.f,
                        static_cast<float>(row) * cChunkManager.chunkSize * cChunkManager.blockSize};
