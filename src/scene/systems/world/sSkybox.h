...
void skyboxSystem(entt::registry& reg) {
  ...
  reg.view<CSkybox, CTime, CTexture, CUUID>().each(
      [&](const CSkybox& cSkybox, const CTime& cTime, CTexture& cTexture, const CUUID& cUUID) {
        if (cTexture.drawMode == CTexture::DrawMode::TEXTURES_BLEND) {
          ...
          if (cTime.currentHour >= cTime.nightStart and cTime.currentHour < cTime.dayTransitionStart) {
            ...
            blendFactor =
                1.f - ((cTime.currentMinute + (cTime.currentHour - cTime.nightTransitionStart) * cTime.fps) / 120.f);
          }
