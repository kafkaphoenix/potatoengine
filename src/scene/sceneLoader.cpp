...
void SceneLoader::loadTextures(const std::shared_ptr<Scene>& scene, const std::shared_ptr<AssetsManager>& manager) {
  for (const auto& [texture, options] : scene->getTextures()) {
    ...
    manager->load<Texture>(texture, options.at("filepath").get<std::string>(), options.at("type").get<std::string>(),
                           flipOption);
...
void SceneLoader::loadPrefabs(const std::shared_ptr<Scene>& scene, const std::shared_ptr<AssetsManager>& manager) {
  for (const auto& [prefab, options] : scene->getPrefabs()) {
    manager->load<Prefab>(prefab, options.at("filepath").get<std::string>(),
...
void SceneLoader::loadChildrenScenes(const std::shared_ptr<Scene>& scene,
                                     const std::shared_ptr<AssetsManager>& manager) {
  // TODO: implement
}

void SceneLoader::loadNormalEntities(const std::shared_ptr<Scene>& scene,
                                     const std::shared_ptr<AssetsManager>& manager) {
...
void SceneLoader::loadLightEntities(const std::shared_ptr<Scene>& scene,
                                    const std::shared_ptr<AssetsManager>& manager) {
...
void SceneLoader::loadCameraEntities(const std::shared_ptr<Scene>& scene,
                                     const std::shared_ptr<AssetsManager>& manager) {
...
void SceneLoader::loadSystemEntities(const std::shared_ptr<Scene>& scene,
                                     const std::shared_ptr<AssetsManager>& manager) {
