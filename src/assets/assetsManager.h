...
class AssetsManager {
  public:
    template <typename Type, typename... Args> void load(std::string_view id, Args&&... args) {
      ENGINE_ASSERT(not m_assets.contains(id.data()), "Asset {} already exists!", id);
    ...
    template <typename Type> const std::shared_ptr<Type>& get(std::string_view id) const {
      ENGINE_ASSERT(contains<Type>(id), "Asset {} not found!", id);
      AssetTypes& asset = m_assets.at(id.data());
      return std::get_if<std::shared_ptr<Type>>(&asset);
    }
    ...
    const std::shared_ptr<Type>& reload(std::string_view id, Args&&... args) {
      ...
      ENGINE_ASSERT(maybeAsset not_eq m_assets.end(), "Asset {} not found!", id);
      ENGINE_ASSERT(std::holds_alternative<std::shared_ptr<Type>>(maybeAsset->second),
    ...
    static std::shared_ptr<AssetsManager> Create() noexcept { return std::make_shared<AssetsManager>(); }

    using AssetTypes = std::variant<std::shared_ptr<Texture>, std::shared_ptr<Shader>, std::shared_ptr<Prefab>,
                                    std::shared_ptr<Model>, std::shared_ptr<Scene>>;
    const std::unordered_map<std::string, AssetTypes>& getAssets() const noexcept { return m_assets; }

    const std::unordered_map<std::string, std::unordered_map<std::string, AssetTypes>>& getAssetsByType() {
      if (not m_isDirty) {
  ...
  private:
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, AssetTypes>> m_assetsByType{};
