...
class Prefab {
  public:
    ...
    const std::unordered_set<std::string>& getInherits(std::string_view prototypeID) const noexcept {
      return m_prototypes.at(prototypeID.data()).inherits;
    }
    const std::unordered_set<std::string>& getCTags(std::string_view prototypeID) const noexcept {
      return m_prototypes.at(prototypeID.data()).ctags;
    }
    const std::unordered_map<std::string, json>& getComponents(std::string_view prototypeID) const noexcept {
    ...
    bool operator==(const Prefab& other) const noexcept { return m_filepath == other.m_filepath; }
  ...
  private:
    ...
    void read(const json& j, std::unordered_set<std::string>& inherits, std::unordered_set<std::string>& ctags,
              std::unordered_map<std::string, json>& components);
};
}