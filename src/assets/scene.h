...
class Scene {
  public:
    ...
    const std::unordered_map<std::string, json>& getNormalEntities() const noexcept { return m_normalEntities; }
    const std::unordered_map<std::string, json>& getLightEntities() const noexcept { return m_lightEntities; }
    const std::unordered_map<std::string, json>& getCameraEntities() const noexcept { return m_cameraEntities; }
    const std::unordered_map<std::string, json>& getSystemEntities() const noexcept { return m_systemEntities; }
    const std::unordered_map<std::string, json>& getFBOEntities() const noexcept { return m_fboEntities; }

    bool operator==(const Scene& other) const noexcept { return m_filepath == other.m_filepath; }
