...
GameState::GameState(std::weak_ptr<engine::AssetsManager> am, std::weak_ptr<engine::Renderer> r, Settings&& s)
    : State("GameState"), m_sceneManager(am, r), m_assetsManager(am), m_renderer(r), m_settings(std::move(s)) {
...
void GameState::onImguiUpdate() { drawDebugger(m_assetsManager, m_renderer, m_sceneManager, m_settings); }

void GameState::onEvent(engine::Event& e) { m_sceneManager.onEvent(e); }

std::unique_ptr<engine::State> GameState::Create(std::weak_ptr<engine::AssetsManager> am,
